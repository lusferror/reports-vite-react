...
const useFetchWithMsal = (msalRequest) => {
    ...
    const { result, error: msalError, acquireToken } = useMsalAuthentication(InteractionType.Popup, {
        ...msalRequest,
    ...
    const execute = async (method, endpoint, data = null) => {
        ...
        if (result || tokenInfo) {
            try {
                let userIfo = result ? result : tokenInfo;
                let response = null;

                const headers = new Headers();
                const bearer = `Bearer ${userIfo.accessToken}`;            
